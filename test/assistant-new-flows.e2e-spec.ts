 ...
 *  (b) the answered-question BUTTON MORPH (R3 / ADR 0058): an `ask_user` turn (a
 *      fresh private `sendActions` keyboard under ADR 0059), then the inline-button
 *      callback edits the ORIGINAL question message — buttons cleared, a localized
 *      "User selected: <label>" line appended — and the resume still opens its own
 *      live surface (its own draft).
 ...
 * harness drives PRIVATE chats by default, so under ADR 0059 each model turn's live
 * surface is an ephemeral native draft and the finalised answer is a FRESH real
 * `sendMessage` (awaited via `nextReply(CapturingVendorConnector.isPrivateAnswer)`),
 * not an `editMessageText` morph. Real streamed deltas are an opt-in manual smoke
 * (E2E_LLM=real), so the user's Anthropic budget is never spent autonomously here.
 */
describe('Assistant new flows (real-request e2e): menu, button-morph, inline recurrence', () => {
  ...
  /**
   * Reads the text off a captured reply. The payload carries a `.text` on every
   * terminal path — a private finalised-answer `sendMessage`, a private `ask_user`
   * `sendActions`, or an answered-question `editMessageText` morph — so `/.../i`
   * patterns match across all of them.
   */
  ...
  /**
   * Reads the inline-button rows off an `ask_user` reply — a fresh `sendActions`
   * (`OutboundActions`) on the private path (ADR 0059) or the morph `editMessageText`
   * keyboard on the non-private fallback. Both payloads expose `.buttons`.
   */
  const buttonsOf = (send: CapturedSend): OutboundActionButton[][] =>
    (send.payload as { buttons: OutboundActionButton[][] }).buttons;

  /**
   * Awaits the PRIVATE-path finalised answer (ADR 0059): the fresh real `sendMessage`
   * (`format === Html`) that supersedes the self-expiring draft. The morph-only
   * `nextSend()` records this as framing and never resolves on it, so the answer is
   * awaited via the predicate seam. Bounded by jest's testTimeout.
   */
  ...
  describe('(b) answered-question button morph (R3 / ADR 0058)', () => {
    it('edits the original question (buttons cleared + "User selected:" appended) on the button tap, and the resume opens its own loader', async () => {
      ...
      // (a draft cannot carry buttons), not an editMessageText morph.
      expect(ask.method).toBe('sendActions');
      expect((ask.payload as OutboundActions).text).toBe(
        'Which day should I book it?',
      );
      ...
      // Await the resume's TERMINAL ANSWER specifically — the continued turn's
      // confirmation ("Done — booked your slot for Friday."). On the private path
      // (ADR 0059) this is a FRESH finalised-answer sendMessage (the draft self
      // -expires), NOT the answered-question morph (an editMessageText carrying
      // clearButtons whose text ALSO contains "Friday"). Identifying it by the
      // private-answer shape AND the answer text guarantees the resumed create_task
      // has committed before the DB assertion.
      const resumeAnswer = await harness.vendor.nextReply(
        (send) =>
          CapturingVendorConnector.isPrivateAnswer(send) &&
          /booked your slot/i.test(replyTextOf(send)),
      );

      expect(resumeAnswer.method).toBe('sendMessage');
      ...
      // FEATURE 2 — the resume still opens its OWN live surface: on the private path
      // (ADR 0059) that surface is an ephemeral native draft, so a fresh
      // `sendMessageDraft` is emitted for the continued turn before its finalised
      // answer lands — the resume is not silent mid-flight.
      const resumeDraft = resumeSends.find(
        (send) => send.method === 'sendMessageDraft',
      );

      expect(resumeDraft).toBeDefined();
