...
The [AI-communication audit](../specs/ai-comms-cc-audit.md) is Accepted (2026-06-20) — a reconciliation of the multi-agent team's audit with the lead engineer's independent findings — and its five CODE-NOW hardenings (A1–A5) are recorded in [ADR 0016](0016-assistant-ai-comms-audit-hardening.md). Separately, the full backlog was tracked as **v1 Foundation** (Stories 1–9, now shipped — as-built in [ai-workflow](../specs/ai-workflow.md)) and **v2 Conversational UX** (Stories 10–18 — now planned in [ai-workflow-v2-plan](../specs/ai-workflow-v2-plan.md)). We need an explicit, recorded order of work and a quality bar so the assistant hardening proceeds predictably rather than ad hoc — particularly because v2 includes a story ([0011](0011-assistant-ai-judged-conflicts.md)) that supersedes an already-accepted ADR.
...
- **Phase B — v1 Foundation (Stories 1–9).** Implement the v1 stories in the **documented suggested order** (now shipped; as-built in [ai-workflow](../specs/ai-workflow.md)): **3 (`AiToolChoice` + the 529 → fallback-model swap — the top failure-handling item)** → **1 (full narration re-drive — the complete classify → corrective user message → forced `tool_choice:'any'` → retry ≤5 → escalate, on top of A1's terminal split)** → 2 (`create_tasks`) → 4 (`buildTool`, carrying the A2 `.describe()` AC + derivation guards) → 6 (router) → 5 (`ask_user`) → 7 (`check_availability`) → 8 (layered decomposition, the vehicle hosting 1–7) → 9 (write-side recurring-conflict gap). Each story is **gated by lint + type + jest** (`pnpm run lint`, `pnpm run type`, `pnpm jest`) before it merges, and **each architectural decision gets its own ADR** (Stories 1, 5 already have ADRs 0009, 0010).
...
- The work this sequences: v1 Stories 1–9 (shipped) as-built in [ai-workflow](../specs/ai-workflow.md); v2 Stories 10–18 in [ai-workflow-v2-plan](../specs/ai-workflow-v2-plan.md)
